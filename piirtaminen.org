...
** Piirtoikkunan pisteen koordinaatit
   ...
   p5.js-ohjelmissa piirretään kuvioita ja animaatioita
   piirtoikkunaan, jota kielessä kutsutaan nimella /canvas/.

   Piirtoikkuna voi olla esimerkiksi selaimen ikkuna tai sen osa.

   Piirtoikkunan eri kohtiin eli /pikseleihin/
  ...
  kaikkien ikkunan kulmapisteiden koordinaatit sekä pisteiden \((70,
   ...
   600 pikseliä leveä ja 400 pikseliä korkea piirtoikkuna luodaan käskyllä ~createCanvas (600, 400);~.

   Seuraava ohjelmakoodi luo piirtoikkunan.
   ...
   Ensimmäisen rivin ~function setup()~ kertoo tietokoneelle,
   että seuraavien aaltosulkeiden ~{}~ sisällä olevat käskyt suoritetaan
   yhden kerran, kun ohjelma käynnistetään. p5.js-ohjelmassa tulee aina olla
   tämänlainen ~setup()~ -niminen /funktio/.

   Jos piirtoikkunan korkeudeksi haluaisikin 500 pikseliä, muutettaisiin toiseksi
   riviksi ~createCanvas (600, 500);~

   Piirtämiseen liittyvät käskyt laitetaan p5.js-ohjelmissa funktioon nimeltä
   ~draw()~. Kaikki piirtämiseen käytettävät käskyt kirjoitetaan samalla
   tavalla aaltosulkeiden ~{}~ sisälle.

   Eräs p5.js-ohjelmien peruskuvioista on suorakulmio (englanniksi
...
** Ohjelmakoodin muoto
   Tietokoneohjelma koostuu tyypillisesti /ohjelmariveistä/.
   p5.js-ohjelma on
   ...
   - p5.js-ohjelmassa komennon ~size()~ täytyy olla ~setup()~ -funktion
     ensimmäinen rivi.

   Kokeile tehdä muutoksia ohjelmaan ja katso, mitä tapahtuu kun yrität
   suorittaa ohjelman.
** Muita kuvioita
   Suorakulmion piirtämiseen käytetään yllä /funktiota/
   ...
   Funktio ~rect()~ piirtää suorakulmion, jonka sivut ovat
...
** Tehtävät
  ...
  4. Piirrä seuraavat kuviot. Piirtoikkunan koko on \(300\times 200.\)
     a) Suorakulmio on piirtoikkunan keskellä, ja suorakulmion leveys
